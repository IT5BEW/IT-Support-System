...
EquipmentID.addEventListener("focusout", CheckForm1)
ComName.addEventListener("focusout", CheckForm1)
User.addEventListener("focusout", CheckForm1)
Section.addEventListener("focusout", CheckForm1)
...
async function fillForm() {
  ...
  if(check1&&check2&&check3){
    ...
    Form_EquipmentID.setText(EquipmentID.value)
    Form_ComName.setText(ComName.value)
    Form_User.setText(User.value)
    Form_Section.setText(Section.value)

    if(ReInstall.checked){Form_CheckBox3.check()}
    if(Broken.checked){Form_CheckBox4.check()}
    if(ETC.checked){Form_CheckBox5.check()}
    Form_fill_2.setText(document.getElementById('ETCText').value)

    // Fill in Cause Text Fields
    Form_Text6.setText(Cause.value)
...
function DownloadForm(){
    if(pdfFile != undefined){
      ...
      confirm("กรุณากรอกฟอร์มด้วยครับ")
    }
}
...
function CheckForm1(){
  var check = true;
  var check1 = CheckForm(EquipmentID);
  var check2 = CheckForm(ComName);
  var check3 = CheckForm(User);
  var check4 = CheckForm(Section);
  var check5 = FixCom.checked;
  var check6 = FixETC.checked;
  if(!((check1&&check2&&check3&&check4)&&(check5||check6))){check = false;}
